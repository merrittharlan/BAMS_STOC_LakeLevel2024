...
LandsatSentinel2_df <- lake_levels_nc(LandsatSentinel2file, altimetry = "Sentinel2")
LandsatGREALM_df <- lake_levels_nc(LandsatGREALMfile, altimetry = "GREALM")
LandsatICESat2_df <- lake_levels_nc(LandsatICESat2file, altimetry = "ICESat2")
...
combined_df_ID = rbind(LandsatSentinel2_df, LandsatGREALM_df, LandsatICESat2_df) %>%
  select(ID, lat, lon, lake_name, country_name, basin_id, basin_name, altimetry) %>%
  distinct()
...
anomaly_calc <- function(rownum){
  ...
  bl = as.numeric(unlist(storage_i %>% filter(Year %in% c(1993:2020)) %>% select(lake_storage)))
...
write.csv(df_out %>% st_drop_geometry(), "out/Glolakes_anomaly_1993_2020BL.csv", row.names = FALSE)
write.csv(combined_data, "out/Glolakes_timeseries_1993_2020BL.csv", row.names = FALSE)